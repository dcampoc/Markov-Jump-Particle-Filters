...
for t = 1:trainlen,
    ...
    D5 = D(:, 5);%I take position in D1
    M5 = M(:, 5);%I take position in M1
    D6 = D(:, 6);%I take position in D2
    M6 = M(:, 6);%I take position in M2
    D7 = D(:,7);%I take x' in D3
    M7 = M(:,7);%I take x' in M3
    D8 = D(:,8);%I take x' in D4
    M8 = M(:,8);%I take x' in M4
    WD1 = 2*diag(mask1)*D1';    % constant matrix
    WD2 = 2*diag(mask2)*D2';
    WD3 = 2*diag(mask3)*D3';
    WD4 = 2*diag(mask4)*D4';
    WD5 = 2*diag(mask5)*D5';    % constant matrix
    WD6 = 2*diag(mask6)*D6';
    WD7 = 2*diag(mask7)*D7';    % constant matrix
    WD8 = 2*diag(mask8)*D8';
    Known = ~isnan(D);
    ...
    known5 = Known(:, 5);%I take position in k
    known6 = Known(:, 6);%I take position in k
    known7 = Known(:,7);%I take x' in k
    known8 = Known(:,8);%I take x' in k
    ...
    W15 = (mask5*ones(1,dlen)) .* known5';
    W16 = (mask6*ones(1,dlen)) .* known6';
    W17 = (mask7*ones(1,dlen)) .* known7';
    W18 = (mask8*ones(1,dlen)) .* known8';
    D(find(~Known)) = 0;
    dconst = beta*((D1.^2)*mask1)'+ beta*((D2.^2)*mask2)'+...
        beta*((D5.^2)*mask5)'+ beta*((D6.^2)*mask6)'+...
        beta*((D7.^2)*mask7)'+ beta*((D8.^2)*mask8)'+...
        alpha*((D3.^2)*mask3)'+ alpha*((D4.^2)*mask4)';                     % Constant in distance calculation for each data sample
    % W2 = ones(munits,1)*mask'; D2 = (D'.^2);
    % Dist = (M.^2)*mask*ones(1,d) + ones(m,1)*mask'*(D'.^2) - 2*M*diag(mask)*D'
    while i0+1<=dlen,
        ...
        Dist = beta*(M1.^2)*W11(:,inds) + beta*(M2.^2)*W12(:,inds) +...
            beta*(M5.^2)*W15(:,inds) + beta*(M6.^2)*W16(:,inds) +...
            beta*(M7.^2)*W17(:,inds) + beta*(M8.^2)*W18(:,inds) +...
            alpha*(M3.^2)*W13(:,inds) + alpha*(M4.^2)*W14(:,inds)...
            - beta*M1*WD1(:,inds) - beta*M2*WD2(:,inds)+...
            - beta*M5*WD5(:,inds) - beta*M6*WD6(:,inds)+...
            - beta*M7*WD7(:,inds) - beta*M8*WD8(:,inds)+...
            - alpha*M3*WD3(:,inds) - alpha*M4*WD4(:,inds);
