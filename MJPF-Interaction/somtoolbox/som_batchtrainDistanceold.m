...
for t = 1:trainlen,  
  ...
  mask1=mask((1:maskside1/2), :);
  mask2=mask((maskside1/2+1:maskside1),:);
  i0 = 0;
  [msize1 msize2]=size(D);%training data i want to find dimensions
  D1=D(:, (1:msize2/2));%I take position in D1
  M1=M(:, (1:msize2/2));%I take position in M1
  D2=D(:,(msize2/2+1: msize2));%I take x' in D2
  M2=M(:,(msize2/2+1: msize2));%I take x' in M2
  WD1 = 2*diag(mask1)*D1';    % constant matrix
  WD2 = 2*diag(mask2)*D2';
  Known = ~isnan(D);
  [ksize1 ksize2]=size(Known);
  known1=Known(:, (1:ksize2/2));%I take position in k
  known2=Known(:,(ksize2/2+1: ksize2));%I take x' in k
  W11 = (mask1*ones(1,dlen)) .* known1'; %W11 saved
  W12 = (mask2*ones(1,dlen)) .* known2';
  D(find(~Known)) = 0;  
  dconst = beta*((D1.^2)*mask1)'+alpha*((D2.^2)*mask2)'; % constant in distance calculation for each data sample 
                         % W2 = ones(munits,1)*mask'; D2 = (D'.^2);
                         % Dist = (M.^2)*mask*ones(1,d) + ones(m,1)*mask'*(D'.^2) - 2*M*diag(mask)*D'
  while i0+1<=dlen, 
    inds = [(i0+1):min(dlen,i0+blen)]; i0 = i0+blen;     
    %distance is M^2-2MD+D^2
    %distance is now beta(M1^2-2M1D1+D1^2) + alpha(M2^2-2M2D2+D2^2) alpha to
    %choose
    %Dist = (M.^2)*W1(:,inds) - M*WD(:,inds);%% distance super pixel
    Dist = beta*(M1.^2)*W11(:,inds) +(M2.^2)*W12(:,inds)*alpha - beta*M1*WD1(:,inds)-alpha*M2*WD2(:,inds);
